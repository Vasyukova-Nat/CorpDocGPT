...
```
2) Скачать модель Ollama (только при первом запуске):
```
docker exec -it ollama ollama pull qwen2.5:0.5b
```
3) Перезапустить бэкенд для применения модели:
```
docker-compose restart backend
```
Станут доступны: веб-приложение (http://localhost:1420), backend (http://localhost:8000), Ollama (http://localhost:11434). 
